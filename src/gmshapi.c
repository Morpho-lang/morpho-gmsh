...
value MorphoGmshOptionRestoreDefaults(vm* v, int nargs, value* args) {
    if (nargs != 0) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
...
value MorphoGmshModelMeshRemoveElements(vm* v, int nargs, value* args) {
    if (nargs != 3) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    ...
    morphoGetSizeTStarFromList(v, elementTags_list, elementTags); 
    int ierr;
    gmshModelMeshRemoveElements(dim,
                                tag,
                                elementTags, sizeof(elementTags)/sizeof(elementTags[0]),
                                &ierr);
    return MORPHO_NIL;
...
value MorphoGmshModelOccDefeature(vm* v, int nargs, value* args) {
    if (nargs != 3) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    ...
    morphoGetIntStarFromList(v, volumeTags_list, volumeTags); 
    if (!MORPHO_ISLIST(MORPHO_GETARG(args, 1))) {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    objectlist* surfaceTags_list = MORPHO_GETLIST(MORPHO_GETARG(args, 1)); 
    int surfaceTags_len = list_length(surfaceTags_list); 
    int surfaceTags[surfaceTags_len]; 
    morphoGetIntStarFromList(v, surfaceTags_list, surfaceTags); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 2)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int removeVolume = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 2)); 
    int* outDimTags;
    size_t outDimTags_n;
    int ierr;
    gmshModelOccDefeature(volumeTags, sizeof(volumeTags)/sizeof(volumeTags[0]),
                          surfaceTags, sizeof(surfaceTags)/sizeof(surfaceTags[0]),
                          &outDimTags, &outDimTags_n,
                          removeVolume,
                          &ierr);
...
value MorphoGmshModelOccFillet2D(vm* v, int nargs, value* args) {
    if (nargs != 4) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 0)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int edgeTag1 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 0)); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 1)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int edgeTag2 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 1)); 
    if (!morpho_is_double(MORPHO_GETARG(args, 2)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const double radius = morpho_get_double(MORPHO_GETARG(args, 2)); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 3)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int tag = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 3)); 
    int ierr;
    int outval = gmshModelOccFillet2D(edgeTag1,
                         edgeTag2,
                         radius,
                         tag,
                         &ierr);
...
value MorphoGmshModelOccChamfer2D(vm* v, int nargs, value* args) {
    if (nargs != 5) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    ...
    const int edgeTag2 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 1)); 
    if (!morpho_is_double(MORPHO_GETARG(args, 2)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const double distance1 = morpho_get_double(MORPHO_GETARG(args, 2)); 
    if (!morpho_is_double(MORPHO_GETARG(args, 3)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const double distance2 = morpho_get_double(MORPHO_GETARG(args, 3)); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 4)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int tag = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 4)); 
    int ierr;
    int outval = gmshModelOccChamfer2D(edgeTag1,
                          edgeTag2,
                          distance1,
                          distance2,
                          tag,
...
value MorphoGmshModelOccOffsetCurve(vm* v, int nargs, value* args) {
    if (nargs != 2) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 0)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int curveLoopTag = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 0)); 
    if (!morpho_is_double(MORPHO_GETARG(args, 1)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const double offset = morpho_get_double(MORPHO_GETARG(args, 1)); 
    int* outDimTags;
    size_t outDimTags_n;
    int ierr;
    gmshModelOccOffsetCurve(curveLoopTag,
                            offset,
                            &outDimTags, &outDimTags_n,
                            &ierr);
    value outDimTags_value[(int) outDimTags_n];
...
value MorphoGmshModelOccGetDistance(vm* v, int nargs, value* args) {
    if (nargs != 4) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 0)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int dim1 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 0)); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 1)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int tag1 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 1)); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 2)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int dim2 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 2)); 
    if (!MORPHO_ISINTEGER(MORPHO_GETARG(args, 3)))  {morpho_runtimeerror(v, GMSH_ARGS_ERROR); return MORPHO_NIL; }
    const int tag2 = MORPHO_GETINTEGERVALUE(MORPHO_GETARG(args, 3)); 
    double distance;
    double x1;
    double y1;
    double z1;
    ...
    gmshModelOccGetDistance(dim1,
                            ...
                            &ierr);
    value outval[7];
    outval[0] = MORPHO_OBJECT(distance);
    outval[1] = MORPHO_OBJECT(x1);
    outval[2] = MORPHO_OBJECT(y1);
    outval[3] = MORPHO_OBJECT(z1);
    outval[4] = MORPHO_OBJECT(x2);
    outval[5] = MORPHO_OBJECT(y2);
    outval[6] = MORPHO_OBJECT(z2);
    objectlist* outlist = object_newlist(7, outval);
    value out;
    if (outlist) {
        out = MORPHO_OBJECT(outlist);
...
value MorphoGmshLoggerGetMemory(vm* v, int nargs, value* args) {
    if (nargs != 0) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    int ierr;
    double outval = gmshLoggerGetMemory(&ierr);
    return MORPHO_FLOAT(outval);
}

value MorphoGmshLoggerGetTotalMemory(vm* v, int nargs, value* args) {
    if (nargs != 0) {
        morpho_runtimeerror(v, GMSH_NARGS_ERROR);
        return MORPHO_NIL;
    } 
    int ierr;
    double outval = gmshLoggerGetTotalMemory(&ierr);
    return MORPHO_FLOAT(outval);
}
...
void gmshapi_initialize(void) {
    ...
    builtin_addfunction(MORPHOGMSHOPTIONRESTOREDEFAULTS_FUNCTION, MorphoGmshOptionRestoreDefaults, BUILTIN_FLAGSEMPTY);
    ...
    builtin_addfunction(MORPHOGMSHMODELMESHREMOVEELEMENTS_FUNCTION, MorphoGmshModelMeshRemoveElements, BUILTIN_FLAGSEMPTY);
    ...
    builtin_addfunction(MORPHOGMSHMODELOCCDEFEATURE_FUNCTION, MorphoGmshModelOccDefeature, BUILTIN_FLAGSEMPTY);
    builtin_addfunction(MORPHOGMSHMODELOCCFILLET2D_FUNCTION, MorphoGmshModelOccFillet2D, BUILTIN_FLAGSEMPTY);
    builtin_addfunction(MORPHOGMSHMODELOCCCHAMFER2D_FUNCTION, MorphoGmshModelOccChamfer2D, BUILTIN_FLAGSEMPTY);
    builtin_addfunction(MORPHOGMSHMODELOCCOFFSETCURVE_FUNCTION, MorphoGmshModelOccOffsetCurve, BUILTIN_FLAGSEMPTY);
    builtin_addfunction(MORPHOGMSHMODELOCCGETDISTANCE_FUNCTION, MorphoGmshModelOccGetDistance, BUILTIN_FLAGSEMPTY);
    ...
    builtin_addfunction(MORPHOGMSHLOGGERGETMEMORY_FUNCTION, MorphoGmshLoggerGetMemory, BUILTIN_FLAGSEMPTY);
    builtin_addfunction(MORPHOGMSHLOGGERGETTOTALMEMORY_FUNCTION, MorphoGmshLoggerGetTotalMemory, BUILTIN_FLAGSEMPTY);
